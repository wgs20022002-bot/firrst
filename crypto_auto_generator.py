...
---
📌 캐시 우드, 또 역발상 매수!
"폭락한 팔란티어(PLTR)에 $1,100만 올인" 💰

모두가 매도할 때 ARK가 85,485주를 쓸어담았는데요! ♥(댓글참고)
---

나쁜 예시:
"Arthur Hayes, 비트코인 최대 위협은 이란 전쟁이 아니다" ← 기사 제목 번역일 뿐, 훅 아님

【답글1 — 핵심 팩트 (2~3문장)】
"누가 뭐라고 했는지 / 무슨 일이 일어났는지"를 구체적으로.
종목명, 인물명, 금액, 수량 반드시 포함. 통칭 금지.

좋은 예시:
"아더 헤이즈는 비트코인이 10만 달러를 못 넘는 이유가 전쟁이 아닌 'AI발 실업'이라 분석함.
이란-이스라엘 긴장보다 AI가 고연봉 일자리를 뺏는 속도가 더 빠름."

나쁜 예시:
"BitMEX 창립자가 밝힌 진짜 리스크는 따로 있다는 메시지" ← 뭔 리스크? 정보 없음

【답글2 — 심층 메커니즘 (2~3문장)】
"왜 이게 중요한지" — 돈의 흐름, 인과관계, 대결/반전 구도.
구체적 숫자와 메커니즘 설명 필수.

좋은 예시:
"연봉 25만 불 이상 엔지니어들이 AI에 밀려 해고되자 이들이 짊어진 막대한 대출이 은행권을 압박함.
이 강력한 디플레이션이 시장의 유동성을 말려버리는 중임."

나쁜 예시:
"전쟁보다 더 근본적인 구조적 문제가 비트코인을 위협한다는 관점" ← 무슨 구조적 문제? 빈 말

【답글3 — 전망 + 킬러 문장 (2~4문장)】
향후 어떻게 될지 + 마지막에 "큰따옴표 킬러 문장"으로 마무리.
확신에 찬 단정 어조(~임, ~함). 추측성 표현 금지.

좋은 예시:
"비트코인이 폭등은 역설적으로 '시스템 붕괴' 이후에 옴.
대량 해고로 경제가 작살나야 연준이 다시 미친 듯이 돈을 찍을 것이고 그때가 비트코인의 진짜 파티 타임임.

\"전쟁은 노이즈일 뿐, AI가 죽인 일자리가 비트코인의 유동성 줄기를 막고 있음\""

【글쓰기 톤】
• 불릿(•) 나열 금지 — 모든 답글은 문장형으로 (2~3문장씩 흐르듯이)
• "~임", "~함" 단정 어조. "~인 것 같습니다" 같은 추측 금지
• 번역투 금지 ("~에 따르면" → 자연스러운 한국어)
• 친근하지만 전문가다운 톤 (반말X, 존댓말X, 해체 서술형)
• 해시태그 사용 금지

═══ 포스트 유형별 가이드 ═══

"single" → 메인 트윗 + 답글 3개 스레드 (위 형식 그대로)
...
"data_post" → 온체인/데이터 포스트
  ...
  답글1: 세부 수치 나열 (ETF: 🟠 BTC / 🔵 ETH 구분 가능)
  답글2: 데이터가 의미하는 것 (돈의 흐름 해석)
  답글3: 킬러 문장
...
• 모든 답글은 반드시 기사에서 추출한 구체적 정보(숫자, 법률명, 이름, 날짜, 금액, 형량)를 담아야 함
• "대폭", "크게", "상당히" 같은 부사 대신 실제 수치를 넣어라 (기사에 있으면 반드시!)
• 불릿(•) 나열 형식 금지 — 문장형으로 자연스럽게 써라
...
위 기사를 바탕으로 [메인] + [답글1] + [답글2] + [답글3] 형식의 X 스레드를 작성하세요.
...
4. 빈 말 금지: "핵심 위협 요소 존재", "화제가 되고 있는 상황" 같은 내용 없는 문장 금지. 모든 답글에 구체적 팩트 필수.
